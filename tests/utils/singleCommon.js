    ...
    describe('when remove is true', () => {
      ...
      describe('when transitionAppear', () => {
        it('should render children', () => {
          expect(TestUtils.scryRenderedDOMComponentsWithTag(instance, 'span')[0]).toBeFalsy();
          const child = TestUtils.findRenderedDOMComponentWithTag(instance, 'div');
          expect(getOpacity(ReactDOM.findDOMNode(child))).toBe(1);
        });
        ...
        it('should anim children', done => {
          ...
          expect(TestUtils.scryRenderedDOMComponentsWithTag(innerInstance, 'span')[0]).toBeFalsy();
          const child = TestUtils.findRenderedDOMComponentWithTag(innerInstance, 'div');
          expect(getOpacity(ReactDOM.findDOMNode(child))).not.toBe(1);
          setTimeout(() => {
            expect(getOpacity(ReactDOM.findDOMNode(child))).toBe(1);
      ...
      describe('when transitionEnter', () => {
        it('should render children', () => {
          expect(TestUtils.scryRenderedDOMComponentsWithTag(instance, 'span')[0]).toBeFalsy();
          const child = TestUtils.findRenderedDOMComponentWithTag(instance, 'div');
          expect($(ReactDOM.findDOMNode(child)).text()).toBe('child element');
        });
      });
      ...
      describe('when toggle transitionEnter', () => {
        it('should remove children after transition', done => {
          ...
          expect(TestUtils.scryRenderedDOMComponentsWithTag(instance, 'div')[0]).toBeTruthy();
          setTimeout(() => {
            expect(TestUtils.scryRenderedDOMComponentsWithTag(instance, 'div')[0]).toBeFalsy();
            done();
          }, 1000);
        });
      });
